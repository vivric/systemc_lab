...
#define REPORT_DEFINE_GLOBALS		// this symbol must be defined only once in the whole project to enable reporting
...
memory::memory(sc_module_name name, sc_core::sc_time read_delay // read delay
		, sc_core::sc_time write_delay // write delay
		, sc_dt::uint64 memory_size // memory size (bytes)
		, unsigned int memory_width // memory width (bytes)
) :
	sc_module(name), m_read_delay(read_delay), m_write_delay(write_delay), m_memory_size(
			memory_size), m_memory_width(memory_width), m_previous_warning(false) {
...
//   
//==============================================================================
void memory::operation(tlm::tlm_generic_payload &gp, sc_core::sc_time &delay_time ///< transaction delay
) {
	...
	default: {
		if (m_previous_warning == false) {
			msg << name() << " Unsupported Command Extension";
...
//   
//==============================================================================
void memory::get_delay(tlm::tlm_generic_payload &gp ///< TLM2 GP reference
	...
	default: {
		if (m_previous_warning == false) {
			msg << name() << " Unsupport GP command extension";
